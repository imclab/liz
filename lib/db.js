...
module.exports = function (url) {
  ...
  var profilesAPI = {
     ...
     * @param {{email: string, role: string, calendars: string, tag: string}} profile
     ...
     * @param {function} callback   Called as callback(err, profile)
     */
    removeAll: function (email, callback) {
      db.profiles.remove({email: email}, callback);
    },
  ...
  var groupsAPI = {
    ...
    /**
     * Get all groups (aggregates all profiles)
    ...
    list: function (options, callback) {
      var mapFn = function () {
        emit(this.role, this.email);
      };

      var reduceFn = function (role, emails) {
        return {
          id: 'group:' + role,
          name: role,
          count: emails.length,
