...
module.exports = function (url) {
  ...
  var groupsAPI = {
     ...
     *                                  Where groups is an array with groups
     */
    get: function (email, callback) {
      db.groups.find({
        query: {email: email}
     ...
     * @param {{email: string, group: string, calendar: string, tag: string}} group
     * @param {function} callback   Called as callback(err, group)
     */
    add: function (group, callback) {
      db.groups.insert(group, function (err, document) {
        if (err) return callback(err, null);
    ...
    /**
     * Create or update a group to a user
     * @param {{email: string, group: string, calendar: string, tag: string}} group
     * @param {function} callback   Called as callback(err, group)
     */
    update: function (group, callback) {
      if (group._id) {
        var changes = _.extend({}, group);
        delete changes._id;
     ...
     *                             {group: string, count: number, members: string[]}
     */
    ...
    list: function (options, callback) {
      ...
      db.groups.mapReduce(mapFn, reduceFn, _options, function (err, result) {
