...
var debug = require('debug')('liz:db');
...
module.exports = function (url) {
  ...
  var users = {
      ...
      db.users.findOne({
        ...
        if (user) {
          ...
          if (new Date(user.auth.expires) < (Date.now() + 5 * 60 * 1000)) {
            ...
            debug('refreshing accessToken of user ' + email + '...'); // TODO: cleanup
            ...
            gutils.refreshAccessToken(user.auth.refreshToken, function (err, result) {
              ...
              if (result.access_token && result.expires_in) {
                ...
                debug('Failed to refresh access token', result); // TODO: cleanup
    ...
    /**
     * Initialize a new user. Extends the user profile with information from
     * google, and creates an individual profile when missing.
     * @param {Object} user
     * @param {function} callback
     */
    initialize: function (user, callback) {
      debug('initializing new user ' + user.email+ '...');

      // add user information like name and picture to the user object
      debug('retrieving info from google for ' + user.email+ '...');
      gutils.getUserInfo(user.auth.accessToken, function (err, userData) {
        ...
        users.update(userData, callback);

        // TODO: create individual profile
      });
    }
  };
