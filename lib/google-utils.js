...
exports.validateProfile = function (profile, accessToken, callback) {
  var now = new Date();
  ...
  function getAvailabilityEvents (calendarId, callback) {
  ...
  var calendarIds = profile.calendars && profile.calendars.split(',') || [];
  async.map(calendarIds, getAvailabilityEvents, function (err, results) {
    var hasAvailabilityEvents = results.some(function (items) {
      return items.length > 0;
    });

    var issues = [];
    if (!hasAvailabilityEvents) {
      issues.push({
        profileId: profile.id,
        type: 'availability',
        message: 'No availability events found'
      });
    }
