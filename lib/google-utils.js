...
var debug = require('debug')('google-utils');
 ...
 * @param {Object} params     An object structured like:
 *                            {}
 * @param {string} timeZone
 * @return {Array} Returns the generated events
 */
exports.generateAvailabilityEvents = function (params, timeZone) {
  if(params.tag === undefined) {
    throw new Error('Property "tag" expected');
  }
  if (!Array.isArray(params.days)) {
    throw new Error('Property "days" expected, should be an Array');
  }
  if(params.zone === undefined) {
    throw new Error('Property "zone" expected with the time zone');
  }

  return params.days.map(function (day) {
    var start = moment(getDate(day.day) + 'T' + formatTime(day.start) + 'Z')
        .add(params.zone, 'minute');

    var end = moment(getDate(day.day) + 'T' + formatTime(day.end) + 'Z')
        .add(params.zone, 'minute');
...
// TODO: comment
// TODO: this function will probably become redundant
exports.getAvailabilityEvents = function (profile, accessToken, callback) {
  var now = new Date();
  var defaultTimeMin = now;
  ...
  var options = {
    ...
    q: profile.tag        // filter events having the availability tag
  };

  var calendarId = profile.available;
  if (calendarId) {
    gcal(accessToken).events.list(calendarId, options, function (err, data) {
      var items = null;
      if (data && data.items) {
        items = data.items.filter(function (item) {
          return item.summary.trim().toLowerCase() === profile.tag.toLowerCase();
        });
      }
    ...
    callback(new Error('Availability calendar missing in profile.'));
  }
};
...
/**
 * Convert Google Calendar events to intervals
 * @param {Array.<Object>} events
 * @return {Array.<{start: moment, end: moment}>} Returns intervals
 */
exports.eventsToIntervals = function (events) {
...
// TODO: comment
exports.createAvailabilityEvents = function (params, accessToken, callback) {
  var calendarId = params.calendar;
  if (!calendarId) {
    return callback(new Error('No calendarId provided'));
  }

  // get timeZone of the selected calendar
  debug('getTimeZone', calendarId);
  exports.getTimeZone(calendarId, accessToken, function (err, timeZone) {
    if (err) return callback(err);

    debug('generateAvailabilityEvents');

    // generate events from the given profile
    try {
      var events = exports.generateAvailabilityEvents(params, timeZone);

      debug('createAvailabilityEvents');
...
exports.createCalendar = function (summary, accessToken, callback) {
  debug('create calendar', summary);
  gcal(accessToken).calendars.insert({summary: summary}, callback);
};
